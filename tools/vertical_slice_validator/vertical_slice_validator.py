...
def _validate_causal_evidence(vertical_id: str, event: dict[str, Any], causal_contract: dict[str, Any], errors: list[dict[str, str]]) -> None:
    """Validate the ACAP causal-evidence boundary for a covered vertical."""
    spec = causal_contract["verticals"].get(vertical_id)
    if spec is None:
        return  # Worker Economy remains governed by the existing vertical registry contract.
    ...
    if not isinstance(causal, dict):
        ...
        return
    for field in causal_contract["required_evidence_fields"]:
    ...
    if state == "CONFIRMED" and (not isinstance(attribution, dict) or not all(attribution.get(p) is True for p in spec["attribution_predicates"])):
    ...
    if event.get("strategic_success") and state not in {"CONFIRMED"}:
        errors.append(_error("VSL-055", "Strategic success cannot be claimed without causal confirmation."))
...
def validate_vertical_slice(trace: dict[str, Any], registry: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if vertical_id in causal_contract["verticals"]:
        pass
    ...
    promotion_eligible = contract.get("status") == "QUALIFICATION_SLICE" and contract.get("candidate") is not True and contract.get("qualification_status") != "NOT_QUALIFIED" and not contract.get("blockers")
    qualified = contract_valid and promotion_eligible and runtime_evidence_seen and any(event.get("stage") == "WORLD_STATE_VERIFIED" and event.get("evidence_type") in {"TARGET_BUILD_RUNTIME", "ENGINE_SPECIFIC"} for event in events)
    return {"qualified": qualified, "contract_valid": contract_valid, "promotion_eligible": promotion_eligible, "vertical_id": vertical_id, "contract_version": contract["version"], "causal_contract_version": causal_contract["schema_version"], "errors": errors}
